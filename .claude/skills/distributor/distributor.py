...
def distribute(project_path: Path, version_suffix: str = "") -> None:
    """version_suffix='' → v1 原行为（bible.json / clues.json / ...）。
    传入 '_v2' → 读 bible_v2.json / clues_v2.json / details_v2.json / mechanics_v2.json，
    输出 character_payloads_v2/ 与 distribution_report_v2.md。纯路径切换，分发逻辑不变。"""
    project_path = Path(project_path)
    s = version_suffix

    # ── Step 1: 读全量 ──────────────────────────────────────────────────────
    bible_path = project_path / f"01_planning/bible{s}.json"
    bible = _read_json(bible_path)
    mechanics_raw = _read_json(project_path / f"02_engineering/mechanics{s}.json")
    clues_raw = _read_json(project_path / f"02_engineering/clues{s}.json")
    details_raw = _read_json(project_path / f"02_engineering/details{s}.json")
    ...
    output_dir = project_path / f"02_engineering/character_payloads{s}"
    ...
    # ── Step 4: 输出分发报告 ────────────────────────────────────────────────
    _write_report(project_path, bible, characters, details, clues, mechanics, errors, s)

    report_name = f"distribution_report{s}.md"
    if errors:
        print(f"❌ 分发完成，但有 {len(errors)} 条断言错误，请检查 {report_name}")
        for e in errors:
            print(f"  · {e}")
        sys.exit(1)
    else:
        print(f"✅ 分发完成，共 {len(characters)} 个角色 payload，断言全部通过")
        print(f"   报告：{project_path / '02_engineering' / report_name}")
...
def _write_report(
    ...
    report_path = project_path / f"02_engineering/distribution_report{s}.md"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python3 distributor.py <project_path> [version_suffix 如 _v2]")
        sys.exit(1)
    suffix = sys.argv[2] if len(sys.argv) >= 3 else ""
    distribute(Path(sys.argv[1]), suffix)
